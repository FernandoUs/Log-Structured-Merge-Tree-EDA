...
public:
          ...
          running(false)
    {
        ...
        for (const string& tableName : existingTables) {
            ...
            indices.primary = new lsm::LSMTree<T>(tableName, globalBudget, pPolicy, 1024, pComp, false);
            indices.secondary = new lsm::LSMTree<T>(tableName, globalBudget, sPolicy, 24, sComp, true);
  ...
  Example Usage:
    ...
    void clearMetrics() {
        for (auto& [tableName, indices] : tableIndices) {
            if(indices.primary) indices.primary->resetMetrics();
            if(indices.secondary) indices.secondary->resetMetrics();
        }
