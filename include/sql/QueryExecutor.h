...
namespace sql
{
    ...
    struct TableSchema
    {
        ...
        string partitioningStrategy = "none";
        TableSchema() = default;
        TableSchema(const string &n) : name(n) {}
        NLOHMANN_DEFINE_TYPE_INTRUSIVE(TableSchema, name, columns, types, spatialColumn, mergePolicy, policyParam, spatialComparator, partitioningStrategy)
    };
    ...
    public:
        ...
        string execute(const string &sql)
        {
                ...
                if (pL != string::npos)
                {
                    ...
                    if (pR + 1 < l.length())
                    {
                        ...
                        while (ss >> word)
                        {
                            ...
                            if (wLower == "partition")
                            {
                                ss >> cmd.create->partitioningName;
                            }
                        }
                    }
                }
            ...
            {
                Command cmd;
                cmd.type = CmdType::INSERT;
                cmd.insert = new InsertCmd();
            ...
            {
                Command cmd;
                cmd.type = CmdType::SELECT;
                cmd.select = new SelectCmd();
                if (l.find("count(") != string::npos)
                    cmd.select->countOnly = true;
        ...
        string executeCreate(const Command &cmd)
        {
            ...
            schema.partitioningStrategy = cmd.create->partitioningName;
            ...
            string partName = schema.partitioningStrategy;
            ...
            {
                bool useHilbert = (schema.spatialComparator == "hilbert");
            ...
            indices.primary = new lsm::LSMTree<T>(pName, globalBudget, pPolicy, 1024, pComp, false, 2, 10, nullptr, 3000000);
            indices.secondary = new lsm::LSMTree<T>(sName, globalBudget, sPolicy, 24, sComp, true, 2, 10, partStrategy, 3000000);
