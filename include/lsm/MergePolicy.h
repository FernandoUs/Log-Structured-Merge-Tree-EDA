...
namespace lsm
{
    ...
    public:
            ...
            size_t maxComponentSize) const
        {
            ...
            {
                components[i]->loadFromDisk(directory);
            ...
            sp::ISpatialComparator<T> *comp) const
        {
            ...
            LSMComponent<T>* newComp = new LSMComponent<T>(targetLevel, dimensions, totalWeight);            newComp->build(mergedRecords);
    ...
    public:
        ConcurrentMergePolicy(size_t maxComps = 30, size_t minLen = 3,
    ...
    public:
        explicit LeveledMergePolicy(size_t ratio = 4, size_t base = 128000)
       ...
       vector<LSMComponent<T> *> selectComponentsToMerge(
            const vector<LSMComponent<T> *> &components) const override
        {
            ...
            for (auto *comp : components)
            {
                size_t lvl = comp->getLevel();
                componentsByLevel[lvl].push_back(comp);
                sizePerLevel[lvl] += comp->size();
            }
            ...
            for (const auto &[level, totalSize] : sizePerLevel)
            {
                ...
                if (totalSize > maxAllowed)
                {
                    ...
                    if (componentsByLevel[level].empty()) continue;
                    
                    LSMComponent<T>* sourceComp = componentsByLevel[level][0];
                    victims.push_back(sourceComp);

                    size_t nextLevel = level + 1;
                    
                    if (componentsByLevel.find(nextLevel) != componentsByLevel.end())
                    {
                        sp::MBR sourceMBR = sourceComp->getMBR();

                        for (auto *targetComp : componentsByLevel[nextLevel])
                        {
                            if (targetComp->getMBR().intersects(sourceMBR))
                            {
                                victims.push_back(targetComp);
                            }
                        }
                    }
    ...
    public:
        static MergePolicy<T> *create(const string &name, int param)
        {
            ...
            {
                return new LeveledMergePolicy<T>(param, 5600000);
            }
