...
public:
    ...
    void releaseSpace(size_t bytesFreed) {
        if (bytesFreed > currentBytes) currentBytes = 0;
        else currentBytes -= bytesFreed;
    }
...
private:
    ...
    void loadMetrics() {
        string metricsFile = "data/" + tableName + "/metrics.json";
        if (!fs::exists(metricsFile)) return;
    ...
    void checkAndMerge() {
        ...
        while (mergePolicy->shouldMerge(diskComponents)) {
            ...
            try {
                string fullPath = tableDir + "/" + mergedComponent->getFilename();
            ...
            for (auto* existing : diskComponents) {
                ...
                if (isVictim) {
                    if (!mergedInserted) {
                        nextDiskComponents.push_back(mergedComponent);
                        mergedInserted = true;
                    }
            ...
            cout << "[MERGE] Éxito. Nuevo componente: " << mergedComponent->getFilename() 
                 << " (" << mergedComponent->size() << " registros)" << endl;
        }
    }
public:
    LSMTree(string name, GlobalBudget* budget, MergePolicy<T>* policy, size_t bytesPerRecord, sp::ISpatialComparator<T>* comp, bool persist = true, size_t dims = 2, size_t maxComponents = 10)
    ...
    ~LSMTree() override {
        ...
        if (mergePolicy) delete mergePolicy;
        if (comparator) delete comparator;
    }
